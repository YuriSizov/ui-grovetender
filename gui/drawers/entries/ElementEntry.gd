...
func _handle_released() -> void:
	...
	match _interacted_area:
		...
		InteractedArea.VISIBILITY_HANDLE:
			if element:
				element.set_visible(not element.is_visible())
...
func set_element(value: UIElement) -> void:
	...
	if element:
		element.editor_selected.disconnect(queue_redraw)
		element.editor_deselected.disconnect(queue_redraw)
		element.visibility_changed.disconnect(_update_visibility_icon)
	
	element = value
	
	if element:
		element.editor_selected.connect(queue_redraw)
		element.editor_deselected.connect(queue_redraw)
		element.visibility_changed.connect(_update_visibility_icon)
...
func _update_entry() -> void:
	...
	_element_name.text = element.element_name
	
	# TODO: Restore the switch when element types are reimplemented.
